...
class PrivilegedObservationCollection:
    ...
    observations: Dict[str, Dict[str, PrivilegedRun]]

    def __init__(self):
        self.observations = {}

    def add_run(self, goal: str, run_id: str, run: PrivilegedRun):
        """Add a privileged run for a specific goal."""
        if goal not in self.observations:
            self.observations[goal] = {}
        self.observations[goal][run_id] = run

    def get_run(self, goal: str, run_id: str) -> Optional[PrivilegedRun]:
        """Get a specific run for a goal."""
        return self.observations.get(goal, {}).get(run_id)

    def get_all_runs(self, goal: str) -> List[PrivilegedRun]:
        """Get all runs for a specific goal."""
        if goal not in self.observations:
            return []
        return list(self.observations[goal].values())
    ...
    def has_goal(self, goal: str) -> bool:
        """Check if a goal exists in the collection."""
        return goal in self.observations

    def get_random(self, goal: str, get_step: bool = False) -> Optional[PrivilegedObservation]:
        """Get a random step from any run for a goal."""
        import random

        if goal not in self.observations:
            return None
        runs = list(self.observations[goal].values())
        if not runs:
            return None
        random_run = random.choice(runs)
        steps = random_run.get_all_steps()
...
class PrivalegedAgent(Agent):
    ...
    def load_privaleged_actions(self, privaleged_actions_path: Path):
        ...
        try:
            ...
            for goal, runs_data in data.items():
                for run_id, steps_data in runs_data.items():
                    run = PrivilegedRun()
                    ...
                    self.privileged_observations.add_run(goal, run_id, run)

        except (FileNotFoundError, json.JSONDecodeError, KeyError) as e:
            warn(f"Failed to load privileged actions from {privaleged_actions_path}: {e}")

    def sample_trajectory(self, goal: str, run_id: str) -> List[PrivilegedObservation]:
        """Sample a trajectory of privileged actions for a specific goal and run."""
        if not self.privileged_observations.has_goal(goal):
            warn(f"No privileged actions found for goal: {goal}")
            return []

        run = self.privileged_observations.get_run(goal, run_id)
        if not run:
            warn(f"No runs found for goal: {goal}, run_id: {run_id}")
            return []
    ...
    def set_goal(self, goal):
        self.goal = goal
        self.trajectory = self.privileged_observations.get_random(goal)
